 ...
 ( function( $, wpLinkL10n, wp ) {
	...
	window.wpLink = {
		...
		init: function() {
            ...
            /* Adding aria label field */
            let html = '<div class="wp-aria-label-field"><label><span>Aria Label</span><input type="text" id="wp-aria-label" name="aria-label" /></div>';
			$(html).insertAfter( '#link-options .wp-link-text-field' );
            ...
            inputs['aria-label'] = $('#wp-aria-label');
		...
		open: function( editorId, url, text, aria ) {
			var ed,
				$body = $( document.body );
		...
		refresh: function( url, text, aria ) {
			var linkText = '';
            ...
            /* If the link being edited is in a WYSIWYG editor */
			if ( wpLink.isMCE() ) {
		...
		mceRefresh: function( searchStr, text ) {
			...
			if ( linkNode.length ) {
				...
				if ( href !== '_wp_link_placeholder' ) {
                    ...
                    inputs["aria-label"].val(linkNode.attr( "aria-label" ));
		...
		getAttrs: function() {
			...
			return {
                ...
                "aria-label" : $.trim( inputs['aria-label'].val() )
			};
		},
		...
		buildHtml: function(attrs) {
            ...
            if ( attrs["aria-label"] !== "" ) {
                html += ' aria-label="' + attrs['aria-label'] + '"';
            }
		...
		mceUpdate: function() {
			...
			var parser = document.createElement( 'a' );
			parser.href = attrs.href;
            parser["aria-label"] = attrs["aria-label"];
